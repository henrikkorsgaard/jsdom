...
class ElementImpl extends NodeImpl {
  ...
  _attrModified(name, value, oldValue) {
    ...
    for(let i in this._mutationObserverRegistry){
      ...
      if(mutationOptions.attributes || mutationOptions.attributeFilter ||  mutationOptions.attributeOldValue){
        if(mutationOptions.attributeFilter && mutationOptions.attributeFilter.indexOf(name) > -1){
            let oldValueForMutationRecord = null;
            if(this._mutationObserverRegistry[i]._options.attributeOldValue){
                oldValueForMutationRecord = oldValue;
            }
            let nameWithoutPrefix = name.indexOf(':') < 0 ? name : name.substring(name.indexOf(':')+1, name.length);//Name cannot contain the prefix (e.g. "xml:lang" . remove xml and keep lang) in order to pass the mutation tests.

            const mutationRecord = new MutationRecord("attributes", this, [], [], null, null, nameWithoutPrefix, null, oldValueForMutationRecord); // to adheare to spec, second last parameter should be this.namespaceURI. This will cause all attr cases to fail

            this._mutationObserverRegistry[i]._enqueueMutationRecord(mutationRecord);
        } else if(!mutationOptions.attributeFilter){
          let oldValueForMutationRecord = null;
          if(this._mutationObserverRegistry[i]._options.attributeOldValue){
              oldValueForMutationRecord = oldValue;
          }
          ...
          const mutationRecord = new MutationRecord("attributes", this, [], [], null, null, nameWithoutPrefix, null, oldValueForMutationRecord); 
