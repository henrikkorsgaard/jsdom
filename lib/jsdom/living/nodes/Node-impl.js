...
class NodeImpl extends EventTargetImpl {
  constructor(args, privateData) {
    ...
    //Array of mutationObservers observing the node.
    this._mutationObserverRegistry = [];
  }
  ...
  insertBefore(newChildImpl, refChildImpl) {
    let insertedChild = this._insertBefore(newChildImpl, refChildImpl);
    this._enqueueMutationRecord([idlUtils.wrapperForImpl(insertedChild)], [], insertedChild.prevSibling || null, insertedChild.nextSibling || null);
    return insertedChild;
  }

  _insertBefore(newChildImpl, refChildImpl) {
    // TODO branding
    ...
    if (newChildImpl.nodeType === NODE_TYPE.DOCUMENT_FRAGMENT_NODE) {
      ...
      while ((grandChildImpl = domSymbolTree.firstChild(newChildImpl))) {
        newChildImpl._removeChild(grandChildImpl);
        this._insertBefore(grandChildImpl, refChildImpl);
      }
      ...
      if (oldParentImpl) {
        oldParentImpl._removeChild(newChildImpl);
      }
  ...
  /**
      @mutationObserver: reference to the mutationObserver object
        - Node-impl uses _enqueueMutationRecord

        childList mutations happen in Node-impl.js _descendantRemoved & _descendantAdded
        attributes mutations happen in Element-impl.js _attrModified
        characterData mutations happen in CharacterData-impl.js  _setData()
  **/

  _registerMutationObserver(mutationObserver){
    this._mutationObserverRegistry.push(mutationObserver);
  }

  _enqueueMutationRecord(addedNodes, removedNodes, prevSibling, nextSibling){

    for(let i in this._mutationObserverRegistry){
      let mutationOptions = this._mutationObserverRegistry[i]._options;
      if(mutationOptions.childList && (addedNodes.length > 0 || removedNodes.length > 0)){
        const mutationRecord = new MutationRecord("childList", this, addedNodes, removedNodes, prevSibling, nextSibling, null, null, null);
  ...
  replaceChild(node, child) {
    let childArray = this._replaceChild(node, child);
    this._enqueueMutationRecord([idlUtils.wrapperForImpl(childArray[0])], [idlUtils.wrapperForImpl(childArray[1])], null, null);
    return childArray[1];
  }
  ...
  _replaceChild(node, child) {
    ...
    return [this._insertBefore(node, child), this._removeChild(child)];
  }
  ...
  removeChild(/* Node */ oldChildImpl) {
    let removedChild = this._removeChild(oldChildImpl);
    this._enqueueMutationRecord([], [idlUtils.wrapperForImpl(removedChild)], null, null);
    return removedChild;

  } // raises(DOMException);

  _removeChild(/* Node */ oldChildImpl) {
    if (this._readonly === true) {
  ...
  appendChild(newChild) {
      let appendedChild = this._appendChild(newChild);
      this._enqueueMutationRecord([idlUtils.wrapperForImpl(appendedChild)], [], null, null);
  ...
  _appendChild(newChild) {
    ...
    //JUST ADDING
    return this._insertBefore(newChild, null);
  }
  ...
  normalize() {
    for (const child of domSymbolTree.childrenIterator(this)) {
      ...
      const nextChild = domSymbolTree.nextSibling(child);
      if (prevChild && prevChild.nodeType === NODE_TYPE.TEXT_NODE && child.nodeType === NODE_TYPE.TEXT_NODE) {
        // merge text nodes
        prevChild.appendData(child.nodeValue);
        this._enqueueMutationRecord([], [idlUtils.wrapperForImpl(this._removeChild(child))], idlUtils.wrapperForImpl(prevChild), idlUtils.wrapperForImpl(nextChild));
      }
    }
  }
  ...
  set textContent(txt) {
    ...
    while (child) {
      ...
      removedNodes.push(idlUtils.wrapperForImpl(this._removeChild(child)));
      child = domSymbolTree.firstChild(this);
    }


    if (txt !== "" && txt !== null) {
      addedNodes.push(idlUtils.wrapperForImpl(this._appendChild(this._ownerDocument.createTextNode(txt))));
    }

    this._enqueueMutationRecord(addedNodes, removedNodes, null, null);
  }
